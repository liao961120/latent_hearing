...
#let normal-size = 12.50002pt
#let large-size = 14.27pt
...
#let ams-article(
    ...
    header: locate(loc => {
      ...
      grid(
        ...
        align(center, upper(
          ...
          if calc.odd(i) { short_title } else { author-string }
        )),
  ...
  v(11pt, weak: true)
  align(center, text(size: small-size, date))

  // Configure paragraph properties.
  set par(first-line-indent: 1.2em, justify: true, leading: 0.58em, spacing: .66em)
  ...
  for author in authors {
    ...
    // Custom
    linebreak()
    line(length: 15%, stroke: (paint: black, thickness: .1pt) )
    // Custom end
    smallcaps(dept-str)
    linebreak()

    if "email" in author [
      _Email address:_ #link("mailto:" + author.email.replace(`\`.text, "")) \
    ]
