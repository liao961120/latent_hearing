...
if (interactive()) {
    a = docopt::docopt(doc, args = list("made/train-type3.RDS",
                                        "made/test-type3.RDS",
                                        "made/fit-type3.RDS",
                                        "recovery.300-type3.pdf", 
                                        "predict.300-type3.pdf") )
} else {
    a = docopt::docopt(doc)
}
# print(a)
...
# 
# # Check recovery
# v = ds %>% filter(rhat > 1.03) %>% .$variable
# library(bayesplot)
# color_scheme_set("viridisC")
# pars = function(...) get_pars(fit$summary(), c(...))$variable
# mcmc_trace(fit$draws(inc_warmup = TRUE)[,,],
#            pars= c(pars("lp__"), v, "S[17]", "delta[17]"),
...
for (i in 1:nrow(d)) {
    ...
    points(x_[1], d$mean[i], col=4, pch=19)
...
for (i in 1:nrow(d)) {
    ...
    x_ = rep(true[i], 2)
    y_ = c(d$q5[i], d$q95[i])
    lines(x_, y_, col=col.alpha(4, sqrt(dat$delta[i]/46) ), lwd=5)
    points(x_[1], d$mean[i], col=4, pch=19)
...
for (i in 1:nrow(d)) {
    x_ = rep(true[i], 2)
    y_ = c(d$q5[i], d$q95[i])
    lines(x_, y_, lwd=5, col=4 |> col.alpha(.3))
    points(x_[1], d$mean[i], pch=19, col=4)
...
for (i in 1:nrow(d)) {
    x_ = rep(true[i], 2)
    y_ = c(d$q5[i], d$q95[i])
    lines(x_, y_, col=col.alpha(4), lwd=5)
}
