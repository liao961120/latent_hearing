...
#figure(image("figs/recovery.svg", width: 103%),
  placement: bottom,
  caption: [
    Recovery of the item parameters from the simulated data in @sim-param. The horizontal axes show true values, and the vertical axes show corresponding posterior estimates. Red dots indicate the posterior means, and bars represent the central 90% posterior densities. True age thresholds are labeled#footnote[Those labeled with $-1$ indicate age-unrestricted items.] near the means in the plots for the gamma ($gamma_j$), Guess ($G_j$), and Slip ($S_j$) parameters. As shown in these plots, the posterior variances of the gamma parameters are _negatively_ associated with age thresholds, while those of the Guess and Slip parameters are _positively_ associated with age thresholds.
  ]
)
<recovery> 

== Parameter recovery <sec-parameter-recovery>
@recovery plots the recovery of the item parameters by comparing the true (i.e., simulated) parameter values (horizontal axis) with their corresponding posterior estimates (vertical axis). The dots indicate posterior means, and the bars represent the central 90% posterior densities. Among the four types of item parameters listed, the age threshold parameters (top-left subplot) are the most reliably recovered, with the posterior means aligning closely with the gray identity line. 
